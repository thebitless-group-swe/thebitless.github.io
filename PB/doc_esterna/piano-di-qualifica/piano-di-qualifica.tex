...
\begin{longtable}{|c|c|c|c|c|p{3.5cm}|}
    ...
    \hline
    \endhead
    2.0.0 & 2026-08-28 & - & - & E. De Piccoli & Approvazione$_G$ per ingresso in baseline$_G$ PB$_G$ \\
    \hline
    1.8.0 & 2026-08-28 & S. Vendramin & A. Marini & - & Aggiunge le valutazioni MPC-CC, MPC-TSR, MPC-DD e MPC-TR e aggiorna MPC-PMS \\
    \hline
    1.7.0 & 2026-08-27 & S. Zecchinato & D. Facco & - & Aggiornamento metriche allo sprint$_G$ 12 \\
    \hline
...
\begin{itemize}
    ...
    \item \textbf{Analisi dei Requisiti v12.0.0}: \\
...
\begin{longtable}{|c|p{4cm}|p{3cm}|p{3cm}|}
    ...
    MPC-TR & Test$_G$ Requirements Coverage & $100\%$ requisiti$_G$ funzionali obbligatori coperti & $100\%$ requisiti$_G$ funzionali coperti \\ \hline
...
TA-002 & L'utente seleziona una porzione di testo e ne richiede il riassunto all'LLM$_G$, scegliendo il livello di sintesi; accettando l'output$_G$, il riassunto sostituisce la porzione selezionata. & UC 62, UC 68 & S \\ \hline
TA-003 & L'utente richiede all'LLM$_G$ una riscrittura del testo scegliendo lo stile da applicare (formale, informale o accademico) e, come intervento distinto, la correzione grammaticale; accettando l'output$_G$, il testo riscritto sostituisce quello di partenza. & UC 64, UC 65 & S \\ \hline
TA-004 & L'utente richiede al sistema$_G$ la traduzione del testo indicando la lingua di destinazione fra quelle supportate; accettando l'output$_G$, la traduzione sostituisce il testo di partenza. & UC 63 & S \\ \hline
...
Le misurazioni che seguono fanno riferimento ai due periodi del progetto: il primo, fase \textbf{RTB}, che intercorre tra l'aggiudicazione del capitolato$_G$, avvenuto il 28 marzo 2026, e la consegna della RTB$_G$ e il secondo, fase \textbf{PB}, che intercorre dalla valutazione di \textit{Semaforo Verde RTB} fino alla consegna PB$_G$.
...
\subsubsection{RTB (Sprint 1-7)}
Come possiamo notare dalla Figura \ref{fig:pv-ev}, il \textit{Planned Value} cresce di pari passo con l'avanzare degli sprint$_G$. Vediamo poi come nel primo sprint$_G$ l'\textit{Earned Value} sia minore rispetto alla stima pianificata: questo è dovuto a una inesperienza nella programmazione del lavoro che, a partire dallo sprint$_G$ 2, è stata migliorata. 
Questo lo si può notare anche dall'accumulo di EV che, dallo sprint$_G$ 3, comincia ad essere maggiore dell'accumulo di PV, che riporta quindi un avanzamento del progetto di poco in anticipo rispetto a quanto pianificato. Nell'ultimo sprint$_G$, che coincide con la prima fase di valutazione RTB$_G$, il lavoro è stato principalmente di palestra e meno lavorativo e per questo risultano valori bassi in PV e EV. Valutazione:
\begin{itemize}
    \item \textbf{Valore accettabile:} PV$>0$€, vincolo$_G$ \textit{rispettato}.
    \item \textbf{Valore ottimo:} PV pari alla pianificazione concordata, vincolo$_G$ \textit{rispettato}.
    \item \textbf{Valore accettabile:} EV$\geq 80\% * PV$, vincolo$_G$ \textit{rispettato}.
    \item \textbf{Valore ottimo:} EV$\geq PV$, vincolo$_G$ \textit{violato} in due sprint$_G$ su sette.
\end{itemize}

\subsubsection{PB (Sprint 8-12)}
Analizzando il grafico in Figura \ref{fig:pv-ev} tra lo Sprint$_G$ 8 e lo Sprint$_G$ 12, si osserva la fase di completamento del progetto, in cui sia il \textit{Planned Value} che l'\textit{Earned Value} convergono progressivamente verso il valore cumulato finale di circa 12.250,00 €. 
...
\begin{itemize}
    ...
    \item \textbf{Valore ottimo:} PV pari alla pianificazione concordata, vincolo$_G$ \textit{rispettato}.
    \item \textbf{Valore accettabile:} EV$\geq 80\% * PV$, vincolo$_G$ \textit{rispettato}.
    \item \textbf{Valore ottimo:} EV$\geq PV$, vincolo$_G$ \textit{rispettato}.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Sprint 1:} In questo sprint$_G$ il progetto si avvia con performance al di sotto del livello di ottimalità. Questa dinamica è del tutto coerente con le difficoltà iniziali di calibrazione del team.
    \item \textbf{Sprint 2:} In questo sprint$_G$ il team recupera il ritardo iniziale con un ottimo slancio portandosi in anticipo sulle consegne. Questo slancio, però, comporta un impiego di risorse economiche superiore al previsto, che porta ad uno scenario di anticipo temporale a fronte di un sovraccosto.
    \item \textbf{Sprint 3:} La tendenza all'accelerazione vista nello scorso sprint$_G$ si consolida generando il picco massimo di vantaggio sulla schedulazione. Questo ritmo veloce produce un ulteriore, seppur contenuto, sforamento del budget$_G$.
    \item \textbf{Sprint 4:} In questo sprint$_G$ il team registra una flessione dal punto di vista temporale, accumulando nuovamente ritardo rispetto al piano ideale. Di contro si rileva un eccellente recupero dell'efficienza economica.
    ...
    \item \textbf{Sprint 6:} In questo sprint$_G$ il progetto registra un leggero calo delle prestazioni dovuto principalmente alla sessione d'esami. Questo ha portato ad un ritardo contenuto e ad un leggero sforamento di budget$_G$.
    \item \textbf{Sprint 7:} In quest'ultima iterazione analizzata notiamo \textit{Schedule Variance} e \textit{Cost Variance} pari a zero. Questo è dovuto dal fatto che questo sprint$_G$ rappresentava la prima fase di revisione RTB$_G$, quindi il carico di lavoro è stato poco e facile da portare a termine. 
\end{itemize}
Valutazione: 
\begin{itemize}
    \item \textbf{Valore accettabile CV:} $CV>0$, vincolo$_G$ \textit{violato} in tre sprint$_G$ su cinque con scostamenti lievi.
    \item \textbf{Valore accettabile SV:} $SV\geq 0$, vincolo$_G$ \textit{violato} in tre sprint$_G$ su cinque con slittamenti consistenti.
\end{itemize}

\subsubsection{PB (Sprint 8-12)}
Di seguito la valutazione, sprint$_G$ per sprint$_G$, della fase visibile in Figura \ref{fig:cv-sv}:
\begin{itemize}
\item \textbf{Sprint 8:} Con la ripresa delle attività di sviluppo$_G$ a pieno regime dopo la revisione, il team registra una performance solida. Le metriche di varianza si assestano, garantendo il rispetto delle tempistiche e un buon controllo sul budget$_G$.
...
\item \textbf{Sprint 12:} Nell'ultimo sprint$_G$ analizzato, la \textit{Schedule Variance} si azzera definitivamente, certificando il completamento del lavoro pianificato senza alcun ritardo finale. La \textit{Cost Variance} si attesta su un valore di chiusura positivo, mostrando il completamento del progetto in efficienza economica.
\end{itemize}
Valutazione: 
\begin{itemize}
    \item \textbf{Valore accettabile CV:} $CV>0$, vincolo$_G$ \textit{violato} in due sprint$_G$ su cinque con scostamenti lievi.
    \item \textbf{Valore accettabile SV:} $SV\geq 0$, vincolo$_G$ \textit{rispettato}.
    \item \textbf{Valore ottimo SV:} $SV > 0$, vincolo$_G$ \textit{violato} in due sprint$_G$ su cinque. 
\end{itemize}
...
\subsubsection{RTB (Sprint 1-7)}
Dall'aggiudicazione del capitolato$_G$ fino alla revisione della RTB$_G$, l'\textit{Estimate At Completion} oscilla attorno al \textit{Budget At Completion}. L'EAC supera il BAC solo in tre occasioni:
\begin{itemize}
    \item \textbf{Sprint 1:} come detto anche nelle altre sezioni, il primo sprint$_G$ ha sofferto dell'inesperienza del gruppo nell'organizzare le task$_G$, dimensionarle e portarle a compimento. Questo ha provocato un leggero sforamento di budget$_G$.
    \item \textbf{Sprint 3:} in questo sprint$_G$ il gruppo si è dovuto confrontare con la revisione dell'\textit{Analisi dei Requisiti}, che ha portato ad un rallentamento del progetto e ad un conseguente minore valore aggiunto.
    \item \textbf{Sprint 6:} in questo sprint$_G$ il gruppo ha dovuto affrontare la revisione interna dei documenti e del PoC$_G$ precedente alla prima fase di revisione RTB$_G$. Questo ha portato ad un carico di lavoro importante con conseguente sforamento di budget$_G$ e leggero ritardo in quanto non si è prodotto un effettivo valore aggiunto.
\end{itemize} 
Valutazione:
\begin{itemize}
    \item \textbf{Valore accettabile:} EAC$\leq 110\%$ budget$_G$, vincolo$_G$ \textit{rispettato}.
    \item \textbf{Valore ottimale:} EAC$\leq$ budget$_G$, vincolo$_G$ \textit{violato} in tre sprint$_G$ su cinque.
\end{itemize}
...
Nello sprint$_G$ 9 e 11, infatti, l'EAC si posiziona di poco sopra al budget$_G$ concordato. Questo è frutto delle naturali oscillazioni di prestazione lavorativa del gruppo durante gli sprint$_G$ finali. 
Nonostante ciò, lo sprint$_G$ 12 chiude in linea con il BAC, assicurando che il progetto si concluda in economia senza alcun rischio$_G$ di sforamento del budget$_G$ originario.
Valutazione:
\begin{itemize}
    \item \textbf{Valore accettabile:} EAC$\leq 110\%$ budget$_G$, vincolo$_G$ \textit{rispettato}.
    \item \textbf{Valore ottimale:} EAC$\leq$ budget$_G$, vincolo$_G$ \textit{violato} in tre sprint$_G$ su cinque.
...
\subsubsection{RTB (Sprint 1-7)}
Il \textit{Task Completion Rate} risulta ottimo in tre sprint$_G$ su cinque. Notiamo subottimalità critica nello sprint$_G$ 1 ($76,9\%$) e una subottimalità lieve nello sprint$_G$ 4 ($92,3\%$). Valutazione:
\begin{itemize}
    \item \textbf{Valore accettabile:} $TCR \geq 85\%$, vincolo$_G$ \textit{violato} nello sprint$_G$ 1.
\end{itemize}

\subsubsection{PB (Sprint 8-12)}
Come notiamo dal grafico in Figura \ref{fig:tcr}, il \textit{Task Completion Rate} risulta ottimo in tutti gli sprint$_G$ finali. Questo è dovuto all'esperienza che il gruppo ha acquisito durante il progetto che ha permesso di gestire carichi di lavoro importanti (e.g. 54 task$_G$ nello sprint$_G$ 10) senza particolari problemi di scheduling. Valutazione:
\begin{itemize}
    \item \textbf{Valore accettabile:} $TCR \geq 85\%$, vincolo$_G$ \textit{rispettato}.
    \item \textbf{Valore ottimale:} $TCR = 100\%$, vincolo$_G$ \textit{rispettato}.
\end{itemize}
\begin{figure}[H]
...
\subsubsection{RTB (Sprint 1-7)}
Il \textit{Task Slippage} risulta ottimo in tre sprint$_G$ su cinque. Notiamo subottimalità critica nello sprint$_G$ 1 ($23,1\%$) e una subottimalità lieve nello sprint$_G$ 4 ($7,7\%$). Valutazione:
\begin{itemize}
    \item \textbf{Valore accettabile:} $TS < 15\%$, vincolo$_G$ \textit{violato} nello sprint$_G$ 1.
\end{itemize}

\subsubsection{PB (Sprint 8-12)}
Ugualmente a quello che succede per l'\textit{MPC-TCR}, negli sprint$_G$ relativi alla PB$_G$, il \textit{Task Slippage} risulta ottimo in tutti e cinque gli sprint$_G$. Questo è dovuto alla maturità organizzativa sviluppata durante il progetto che ha permesso di gestire tutti i flussi di lavoro senza particolari problemi. Valutazione:
\begin{itemize}
    \item \textbf{Valore accettabile:} $TS < 15\%$, vincolo$_G$ \textit{rispettato}.
    \item  \textbf{Valore ottimale:} $TS = 0\%$, vincolo$_G$ \textit{rispettato}.
\end{itemize}
\begin{figure}[H]
...
\subsubsection{RTB (Sprint 1-7)}
Dal grafico in Figura \ref{fig:mr} possiamo notare come la valutazione di \textit{Merge Rework}, ovvero la percentuale di Pull Request$_G$ approvate nelle quali viene sovrascritto del lavoro già svolto, varia a seconda della fase del progetto. Nei primi due sprint$_G$ troviamo dei valori simili ($7\%$ circa) che riteniamo accettabili. Questi valori sono dati dal fatto che tutti i membri del gruppo non avevano avuto ancora esperienza di co-working con lo strumento GitHub$_G$, né con un progetto con un gruppo così numeroso. Questo ha portato a iniziali incomprensioni sul lavoro da svolgere e, di conseguenza, a riscritture di parti di documenti. Nel terzo e quarto sprint$_G$ abbiamo valori critici, dovuti principalmente all'\textit{Analisi dei Requisiti} (primo grande documento condiviso da più redattori) e dalla sua riscrittura, dovuta a revisioni a seguito di numerosi difetti di modellazione riscontrati.
Nello sprint$_G$ 5 abbiamo avuto un'enorme mole di Pull Request$_G$ dovute all'inizio del \textit{Proof of Concept} che però sono state minimamente sovrascritte grazie ad una progettazione adeguata. Infine negli ultimi due sprint$_G$ la percentuale di lavoro sovrascritto è stata pari a zero. Questo è dovuto al carico di lavoro diminuito e ad una consapevolezza aumentata sulle task$_G$ da svolgere e sugli strumenti da utilizzare. Valutazione:
\begin{itemize}
    \item \textbf{Valore accettabile:} $MR \leq 20\%$, vincolo$_G$ \textit{violato} in due sprint$_G$ su cinque.
\end{itemize}

\subsubsection{PB (Sprint 8-12)}
Come possiamo notare in Figura \ref{fig:mr}, la percentuale di Pull Request$_G$ approvate nelle quali viene sovrascritto del lavoro già svolto, a differenza di quanto è avvenuto nella fase RTB$_G$, rimane stabile sullo zero durante tutta la fase di PB$_G$. Questo risultato, così come quelli raggiunti per \textit{MPC-TCR} e \textit{MPC-TS}, denotano una maturità nella pianificazione delle task$_G$ e nello svolgere il proprio lavoro, che ci ha portato a non perdere più tempo nel mettere nuovamente mano a ciò che era già stato fatto ma nel proseguire spediti verso la fine.
Valutazione:
\begin{itemize}
    \item \textbf{Valore accettabile:} $MR \leq 20\%$, vincolo$_G$ \textit{rispettato}.
    \item \textbf{Valore ottimale:} $MR \leq 5\%$, vincolo$_G$ \textit{rispettato}.
\end{itemize}
...
\subsubsection{RTB (Sprint 1-7)}
Come è possibile notare in Figura \ref{fig:ir}, l'andamento dell'\textit{Issue Reopening Rate} è altalenante sebbene si mantenga quasi sempre abbondantemente sotto la soglia di accettabilità. L'unico sprint$_G$ che fa eccezione è il numero 3 che, per motivi già affrontati in questo documento, possiede un valore di MPC-IR critico. Valutazione:
\begin{itemize}
    \item \textbf{Valore accettabile:} $IR \leq 15\%$, vincolo$_G$ \textit{violato} nello sprint$_G$ 3.
\end{itemize}

\subsubsection{PB (Sprint 8-12)}
Come per \textit{MPC-TCR}, \textit{MPC-TS} e \textit{MPC-MR}, è possibile notare in Figura \ref{fig:ir} che l'andamento dell'\textit{Issue Reopening Rate} è stabile sullo 0\%. Come per le altre metriche precedentemente citate, il motivo è riscontrabile nella maggiore maturità guadagnata nella durata del progetto. In nessuno sprint$_G$ della fase PB$_G$, infatti, si è reso necessario riaprire issues per svolgere nuovamente un'attività conclusa.
Valutazione:
\begin{itemize}
    \item \textbf{Valore accettabile:} $IR \leq 15\%$, vincolo$_G$ \textit{rispettato}.
    \item \textbf{Valore ottimale:} $IR = 0\%$, vincolo$_G$ \textit{rispettato}.
\end{itemize}
...
In Figura \ref{fig:ig} possiamo vedere il risultato dell'indice di Gulpease di ogni documento prodotto o aggiornato in fase di PB$_G$ e valutato nel suo stato finale (versione $\geq$ 1.0.0). Come possiamo notare, tutti i documenti si posizionano sopra la soglia di ottimalità. L'unico documento che riporta un punteggio minore in fase PB$_G$ rispetto a quello riscontrato in fase RTB$_G$ è il \textit{Glossario}. Questa diminuzione riteniamo sia dovuta all'aggiornamento dei termini rappresentati che hanno inevitabilmente allungato il documento. Purtroppo questo documento predilige per definizione periodi lunghi e termini tecnici che alzano inevitabilmente il livello di difficoltà di comprensione. Il \textit{Glossario} rimane però, come tutti gli altri documenti, sopra la soglia di ottimalità definita dal presente documento.
Valutazione:
\begin{itemize}
    \item \textbf{Valore accettabile:} $IG \geq 40$, vincolo$_G$ sempre \textit{rispettato}.
    \item \textbf{Valore ottimo:} $IG \geq 60$, vincolo$_G$ sempre \textit{rispettato}.
\end{itemize}
...
In Fugura \ref{fig:co} possiamo vedere il grafico che mostra il livello di correttezza ortografica dei documenti redatti o aggiornati e approvati in fase di PB$_G$ (versione $\geq$ 1.0.0). Notiamo come tutti i documenti si posizionino sopra la soglia di ottimalità in quanto non sono presenti nei documenti errori gravi che, se presenti, vengono corretti in fase di revisione. 
Per quanto riguarda gli errori non gravi questi sono rappresentati principalmente da termini tecnici o inglesismi che permeano il progetto. Valutazione:
\begin{itemize}
    \item \textbf{Valore accettabile:} documenti senza errori gravi, vincolo$_G$ \textit{rispettato}.
    \item \textbf{Valore ottimo:} $CO \geq 99\%$, vincolo$_G$ \textit{rispettato}.
\end{itemize}
...
\subsection{Valutazione MPC-CC}
\begin{table}[H]
    \centering
	\begin{tabular}{|c|l|l|l|p{2.4cm}|}
		\hline
		\textbf{Sprint} & \textbf{Istruzioni coperte} & \textbf{Istruzioni totali} & \textbf{CC (\%)} & \textbf{Giudizio} \\
		\hline
		8 & 355 & 899 & 39,5\% & Critico \\
		\hline
        9 & 603 & 990 & 60,9\% & Critico \\
        \hline
        10 & 1.246 & 1.406 & 88,6\% & Accettabile \\
        \hline
        11 & 1.310 & 1.453 & 90,2\% & Ottimo \\
        \hline
	\end{tabular}
	\caption{Copertura del codice per sprint}
	\label{tab:cc}
\end{table}
\subsubsection{PB (Sprint 8--11)}
La metrica è un unico rapporto fra istruzioni coperte e istruzioni totali, e per questo i due lati del sistema$_G$ concorrono al numeratore e al denominatore anziché a due percentuali da mediare: il backend$_G$ è misurato con \texttt{pytest-cov}, il frontend$_G$ con \texttt{vitest} e il fornitore \texttt{v8}. Su tutte e quattro le rilevazioni è stata applicata la configurazione di copertura dell'MVP$_G$, perché il \textit{Proof of Concept}$_G$ non ne aveva alcuna e quella introdotta nel nono sprint$_G$ non dichiarava l'elenco esplicito dei sorgenti: senza quell'elenco il fornitore \texttt{v8} strumenta i soli file caricati dai test$_G$, un sorgente che nessun test$_G$ importa non compare come zero ma semplicemente non compare, e il denominatore si restringe da sé.

Il valore dell'ottavo sprint$_G$ è quello del prototipo. Il \textit{Proof of Concept}$_G$ aveva test$_G$ automatici su entrambi i lati, ma coprivano i percorsi che era stato costruito per dimostrare, ovvero l'analisi dello \textit{streaming}$_G$, il client verso il servizio LLM$_G$ e la rotta di riassunto, e lasciavano fuori tutto ciò che riguardava l'interfaccia. Lo scarto è concentrato nel frontend$_G$, con 117 istruzioni coperte su 616, perché i file di test$_G$ erano tre e nessuno di essi toccava un componente.

La crescita fra il nono e il decimo sprint$_G$ è l'effetto di due decisioni che agiscono insieme. La prima è l'introduzione della \textit{Continuous Integration}$_G$ su GitHub$_G$ Actions$_G$, con la soglia del $70\%$ scritta nella configurazione della pipeline e l'attività chiusa il 3 agosto 2026: da quel momento una pull request$_G$ che abbassa la copertura sotto la soglia non è più integrabile. La seconda è la ripartizione del ruolo Verificatore$_G$ su quattro membri nel decimo sprint$_G$, che ha portato la copertura ben oltre il minimo preteso dalla pipeline. L'undicesimo sprint$_G$ supera la soglia ottimale con un margine sottile, e lo scarto residuo sta tutto nel frontend$_G$, dove i componenti che si limitano a comporre la disposizione della pagina non hanno un test$_G$ proprio. Valutazione:
\begin{itemize}
    \item \textbf{Valore accettabile:} $CC \geq 70\%$, vincolo$_G$ \textit{rispettato} dallo sprint$_G$ 10.
    \item \textbf{Valore ottimale:} $CC \geq 90\%$, vincolo$_G$ \textit{rispettato} nello sprint$_G$ 11.
\end{itemize}

\begin{figure}[H]
    \includegraphics[width=1\textwidth]{charts/cc.pdf}
    \caption{Valutazione \textit{Code Coverage}}
    \label{fig:cc}
\end{figure}

\subsection{Valutazione MPC-TSR}
\begin{table}[H]
    \centering
	\begin{tabular}{|c|l|l|l|p{2.4cm}|}
		\hline
		\textbf{Sprint} & \textbf{Test$_G$ eseguiti} & \textbf{Test$_G$ superati} & \textbf{TSR (\%)} & \textbf{Giudizio} \\
		\hline
		8 & 101 & 100 & 99,0\% & Accettabile \\
		\hline
        9 & 227 & 227 & 100,0\% & Ottimo \\
        \hline
        10 & 644 & 644 & 100,0\% & Ottimo \\
        \hline
        11 & 702 & 702 & 100,0\% & Ottimo \\
        \hline
	\end{tabular}
	\caption{Test$_G$ automatici superati per sprint}
	\label{tab:tsr}
\end{table}
\subsubsection{PB (Sprint 8--11)}
La suite cresce di sette volte fra i due estremi, da 101 a 702 casi, e dal nono sprint$_G$ non registra alcun fallimento. Il valore massimo non è però l'effetto automatico di un prodotto stabile, ma della regola introdotta insieme alla \textit{Continuous Integration}$_G$: dal nono sprint$_G$ una pull request$_G$ con la suite in rosso non è integrabile, e un fallimento vive quindi al più quanto il branch$_G$ che lo ha prodotto. Va letto in questa chiave anche il confronto con MPC-DD, perché i nove difetti censiti nel decimo sprint$_G$ non lasciano qui alcuna traccia: sono emersi dalla revisione fra pari e sono stati corretti sul proprio branch$_G$ prima dell'integrazione$_G$.

L'unico valore inferiore al massimo appartiene al prototipo e discende da un solo caso di prova. Il test$_G$ che verificava la generazione del testo invocava la rotta senza sostituire il client verso il servizio LLM$_G$, e la chiamata raggiungeva il gateway$_G$ reale: su qualunque macchina priva di chiave la risposta era un errore di indisponibilità del fornitore. Il nono sprint$_G$ lo ha corretto con una sostituzione esplicita della dipendenza, annotando nel codice la ragione della modifica.

Il valore dell'ottavo sprint$_G$ va preso per quello che è, ovvero il caso migliore. La suite del prototipo portava anche un secondo difetto, nell'importazione del modulo di \textit{smoke test}, che nominava il pacchetto dei test$_G$ con un prefisso valido soltanto fuori dalla cartella del backend$_G$: eseguendo \texttt{pytest} come prescriveva il file di istruzioni del prototipo la raccolta si interrompeva e nessun test$_G$ del backend$_G$ veniva eseguito, il che ridurrebbe il valore a 40 test$_G$ superati su 101. La rilevazione riportata in tabella è stata quindi condotta risolvendo quell'importazione, come ha poi fatto la correzione del 29 luglio 2026, così da misurare lo stato di salute dei test$_G$ e non quello del comando che li avviava. Valutazione:
\begin{itemize}
    \item \textbf{Valore accettabile:} $TSR \geq 95\%$, vincolo$_G$ \textit{sempre rispettato}.
    \item \textbf{Valore ottimale:} $TSR = 100\%$, vincolo$_G$ \textit{rispettato} dallo sprint$_G$ 9.
\end{itemize}

\begin{figure}[H]
    \includegraphics[width=1\textwidth]{charts/tsr.pdf}
    \caption{Valutazione \textit{Test Success Rate}}
    \label{fig:tsr}
\end{figure}

\subsection{Valutazione MPC-DD}
\begin{table}[H]
    \centering
    \small
    \setlength{\tabcolsep}{4pt}
	\begin{tabular}{|c|>{\centering\arraybackslash}p{1.9cm}|>{\centering\arraybackslash}p{2.1cm}|>{\centering\arraybackslash}p{1.3cm}|>{\centering\arraybackslash}p{2.2cm}|>{\centering\arraybackslash}p{2cm}|}
		\hline
		\textbf{Sprint} & \textbf{Difetti rilevati} & \textbf{Componenti} & \textbf{DD} & \textbf{Difetti aperti a fine sprint} & \textbf{Giudizio} \\
		\hline
		8 & 0 & 9 & 0,00 & 0 & Ottimo \\
		\hline
        9 & 0 & 9 & 0,00 & 0 & Ottimo \\
        \hline
        10 & 9 & 10 & 0,90 & 3 & Accettabile \\
        \hline
        11 & 0 & 10 & 0,00 & 0 & Ottimo \\
        \hline
	\end{tabular}
	\caption{Densità dei difetti per componente per sprint}
	\label{tab:dd}
\end{table}
\subsubsection{PB (Sprint 8--11)}
Lo zero dei primi due sprint$_G$ non descrive un prodotto privo di difetti, ma un periodo in cui nessuno li stava cercando nel codice. L'ottavo sprint$_G$ non prevedeva alcun Programmatore$_G$ fra i ruoli, poiché la progettazione di dettaglio era ancora in avvio, e il codice in vigore restava quello del \textit{Proof of Concept}$_G$; il nono ha aperto il repository$_G$ dell'MVP$_G$ con issue$_G$ che erano tutte compiti di implementazione$_G$. La metrica segue il censimento delle issue$_G$ etichettate come difetto, e dove la verifica$_G$ sul codice non è in corso il censimento resta vuoto.

Tutti e nove i difetti della \textit{Product Baseline}$_G$ cadono nel decimo sprint$_G$, e la coincidenza ha un nome preciso. È lo sprint$_G$ in cui il ruolo Verificatore$_G$ è stato ripartito su quattro membri anziché concentrato su una o due figure, decisione assunta nella riunione di apertura del 4 agosto 2026 in risposta diretta al rischio$_G$ RT-3, \textit{Presenza di errori nel codice}, la cui pericolosità era stata elevata ad \textit{Alta} al termine del nono sprint$_G$. Moltiplicare la capacità di revisione ha moltiplicato i rilievi: sei dei nove sono stati aperti nello stesso giorno, il 6 agosto, e riguardano in prevalenza il backend$_G$, appena toccato dalla riorganizzazione secondo l'architettura$_G$ esagonale.

La densità resta ampiamente entro la soglia di accettabilità perché i nove rilievi si distribuiscono sui componenti anziché concentrarsi in uno solo, ed è esattamente la concentrazione anomala che la metrica serve a intercettare. Sei di essi sono stati chiusi entro il giorno successivo; i tre residui, tutti sul lato frontend$_G$, sono stati chiusi il 16 agosto, dentro l'undicesimo sprint$_G$, che la pianificazione aveva dedicato proprio alla correzione dei difetti emersi in fase di verifica$_G$. Il decimo sprint$_G$ è l'unico periodo in cui la condizione di ottimalità non risulta soddisfatta, perché essa guarda ai difetti ancora aperti alla chiusura del periodo e non a quelli rilevati. Valutazione:
\begin{itemize}
    \item \textbf{Valore accettabile:} $DD \leq 2$ difetti per componente, vincolo$_G$ \textit{sempre rispettato}.
    \item \textbf{Valore ottimale:} $0$ difetti aperti, vincolo$_G$ \textit{violato} nello sprint$_G$ 10.
\end{itemize}

\begin{figure}[H]
    \includegraphics[width=1\textwidth]{charts/dd.pdf}
    \caption{Valutazione \textit{Defect Density}}
    \label{fig:dd}
\end{figure}

\subsection{Valutazione MPC-TR}
\begin{table}[H]
    \centering
    \small
    \setlength{\tabcolsep}{4pt}
	\begin{tabular}{|c|>{\centering\arraybackslash}p{2.6cm}|>{\centering\arraybackslash}p{1.9cm}|>{\centering\arraybackslash}p{2.6cm}|>{\centering\arraybackslash}p{1.9cm}|}
		\hline
		\textbf{Sprint} & \textbf{Requisiti$_G$ obbligatori coperti (\%)} & \textbf{Giudizio} & \textbf{Requisiti$_G$ coperti (\%)} & \textbf{Giudizio} \\
		\hline
		8 & 63,3\% & Critico & 57,1\% & Critico \\
		\hline
        9 & 63,3\% & Critico & 57,1\% & Critico \\
        \hline
        10 & 63,3\% & Critico & 57,1\% & Critico \\
        \hline
        11 & 100,0\% & Ottimo & 100,0\% & Ottimo \\
        \hline
	\end{tabular}
	\caption{Copertura dei requisiti$_G$ da parte dei test$_G$ di sistema$_G$ per sprint}
	\label{tab:tr}
\end{table}
\subsubsection{PB (Sprint 8--11)}
La metrica misura una proprietà del presente documento e non del prodotto, poiché conta i requisiti$_G$ per i quali la tabella dei test$_G$ di sistema$_G$ specifica almeno un test$_G$, qualunque ne sia lo stato. Si distingue perciò da MPD-CRO, che conta i requisiti$_G$ realizzati: qui si misura se il requisito$_G$ è stato messo alla prova, non se la prova è stata superata, ed è la ragione per cui i requisiti$_G$ opzionali concorrono anche quando il test$_G$ corrispondente resta \textit{Non Implementato}. Il denominatore sono i 119 requisiti$_G$ funzionali dell'\textit{Analisi dei Requisiti}$_G$, perché un test$_G$ di sistema$_G$ esercita il comportamento del sistema$_G$, mentre i requisiti$_G$ di qualità e di vincolo$_G$ riguardano il processo, la documentazione e le tecnologie adottate e sono verificati$_G$ per altra via, ovvero dalla pipeline di \textit{Continuous Integration}$_G$, dalla revisione fra pari sulle pull request$_G$ e dalla verifica$_G$ dei documenti.

Il valore resta fermo per tre sprint$_G$ perché la tabella dei test$_G$ di sistema$_G$ è rimasta quella redatta per la RTB$_G$, ovvero gli stessi settanta test$_G$, che tracciavano sessantotto requisiti$_G$ funzionali su centodiciannove. La tabella era stata redatta il 30 maggio 2026 sulla numerazione dei requisiti$_G$ di allora e non è stata riallineata mentre l'\textit{Analisi dei Requisiti}$_G$ attraversava le proprie revisioni, fino a quella dell'ottavo sprint$_G$ che ha recepito i rilievi del prof. Riccardo Cardin: al termine del decimo sprint$_G$ diversi riferimenti citavano ancora la numerazione precedente, come R-3-F-Ob per il taglio di una porzione di testo, che nel frattempo era diventato R-4-F-De.

Il riallineamento è arrivato il 15 agosto 2026, dentro l'undicesimo sprint$_G$, il periodo che la pianificazione aveva dedicato alla verifica$_G$ accurata di tutti i documenti destinati alla \textit{Product Baseline}$_G$. Sono stati aggiunti ventisei test$_G$, da TS-071 a TS-096, ne sono stati rimossi due e tutti i riferimenti esistenti sono stati riscritti sulla numerazione corrente. Da quella revisione ogni requisito$_G$ funzionale ha almeno un test$_G$ di sistema$_G$ che lo nomina, e la metrica passa in un solo periodo dal valore più basso dell'intera \textit{Product Baseline}$_G$ a quello ottimale. È anche la ragione per cui, fra le iniziative di miglioramento della Sezione \ref{sec:iniziative-miglioramento}, il controllo incrociato fra tabelle e definizioni è stato reso obbligatorio per il ruolo Verificatore$_G$. Valutazione:
\begin{itemize}
    \item \textbf{Valore accettabile:} $TR = 100\%$ dei requisiti$_G$ obbligatori coperti, vincolo$_G$ \textit{rispettato} nello sprint$_G$ 11.
    \item \textbf{Valore ottimale:} $TR = 100\%$ dei requisiti$_G$ coperti, vincolo$_G$ \textit{rispettato} nello sprint$_G$ 11.
\end{itemize}

\begin{figure}[H]
    \includegraphics[width=1\textwidth]{charts/tr.pdf}
    \caption{Valutazione \textit{Test Requirements Coverage}}
    \label{fig:tr}
\end{figure}
...
\begin{itemize}
    \item \textbf{Valore accettabile:} $CRO = 100\%$, vincolo$_G$ \textit{rispettato} nello sprint$_G$ 11.
    \item \textbf{Valore ottimale:} $CRO = 100\%$, vincolo$_G$ \textit{rispettato} nello sprint$_G$ 11.
    \item \textbf{Valore accettabile:} $CRD \geq 50\%$, vincolo$_G$ \textit{rispettato} in tre sprint$_G$ su quattro.
    \item \textbf{Valore ottimale:} $CRD \geq 80\%$, vincolo$_G$ \textit{violato}.
    \item \textbf{Valore accettabile:} $CROp \geq 0\%$, vincolo$_G$ \textit{sempre rispettato}.
    \item \textbf{Valore ottimale:} $CROp \geq 60\%$, vincolo$_G$ \textit{violato}.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Valore accettabile:} $CLLM = 100\%$, vincolo$_G$ \textit{rispettato} nello sprint$_G$ 11.
    \item \textbf{Valore ottimale:} $CLLM = 100\%$, vincolo$_G$ \textit{rispettato} nello sprint$_G$ 11.
\end{itemize}
...
\end{table}
\subsubsection{PB (Sprint 8--11)}
Il rendering Markdown$_G$ era già solido nel \textit{Proof of Concept}$_G$, difatti intestazioni, stili del testo, elenchi, citazioni, tabelle, formule matematiche e blocchi di codice venivano resi correttamente fin dal prototipo, che nasceva proprio per dimostrare la fattibilità$_G$ dell'editor.
...
\begin{itemize}
    \item \textbf{Valore accettabile:} $CMD \geq 95\%$ dei casi previsti, vincolo$_G$ \textit{sempre rispettato}.
    \item \textbf{Valore ottimale:} $CMD = 100\%$ dei casi previsti, vincolo$_G$ \textit{rispettato} nello sprint$_G$ 11.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Valore accettabile:} $SN = 100\%$ dei casi obbligatori, vincolo$_G$ \textit{rispettato} nello sprint$_G$ 11.
    \item \textbf{Valore ottimale:} $SN = 100\%$ dei casi obbligatori, vincolo$_G$ \textit{rispettato} nello sprint$_G$ 11.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Valore accettabile:} $0$ perdite nei casi principali, vincolo$_G$ \textit{rispettato} nello sprint$_G$ 11.
    \item \textbf{Valore ottimale:} $0$ perdite in ogni test$_G$, vincolo$_G$ \textit{rispettato} nello sprint$_G$ 11.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Valore accettabile:} $EH \geq 90\%$ degli errori gestiti, vincolo$_G$ \textit{rispettato} nello sprint$_G$ 11.
    \item \textbf{Valore ottimale:} $EH = 100\%$ degli errori gestiti, vincolo$_G$ \textit{violato}.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Valore accettabile:} $AR \geq 95\%$ durante i test$_G$, vincolo$_G$ \textit{sempre rispettato}.
    \item \textbf{Valore ottimale:} $AR \geq 99\%$ durante i test$_G$, vincolo$_G$ \textit{sempre rispettato}.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Valore accettabile:} $LT \leq 30$ minuti, vincolo$_G$ \textit{sempre rispettato}.
    \item \textbf{Valore ottimale:} $LT \leq 10$ minuti, vincolo$_G$ \textit{rispettato} nello sprint$_G$ 11.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Valore accettabile:} $NE \leq 3$ errori per scenario, vincolo$_G$ \textit{sempre rispettato}.
    \item \textbf{Valore ottimale:} $NE = 0$ errori per scenario, vincolo$_G$ \textit{violato}.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Valore accettabile:} $UI \geq 90\%$ degli elementi coerenti, vincolo$_G$ \textit{sempre rispettato}.
    \item \textbf{Valore ottimale:} $UI = 100\%$ degli elementi coerenti, vincolo$_G$ \textit{violato}.
    \item \textbf{Valore accettabile:} $FU \geq 80\%$ delle funzioni comprese senza guida, vincolo$_G$ \textit{rispettato} in tre sprint$_G$ su quattro.
    \item \textbf{Valore ottimale:} $FU \geq 95\%$ delle funzioni comprese senza guida, vincolo$_G$ \textit{rispettato} nello sprint$_G$ 11.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Valore accettabile:} $RT \leq 2$ secondi, vincolo$_G$ \textit{sempre rispettato}.
    \item \textbf{Valore ottimale:} $RT \leq 0{,}5$ secondi, vincolo$_G$ \textit{sempre rispettato}.
    \item \textbf{Valore accettabile:} $MRT \leq 1$ secondo, vincolo$_G$ \textit{sempre rispettato}.
    \item \textbf{Valore ottimale:} $MRT \leq 0{,}3$ secondi, vincolo$_G$ \textit{sempre rispettato}.
    \item \textbf{Valore accettabile:} $LS \leq 3$ secondi, vincolo$_G$ \textit{sempre rispettato}.
    \item \textbf{Valore ottimale:} $LS \leq 1$ secondo, vincolo$_G$ \textit{sempre rispettato}.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Valore accettabile:} $LLMRT \leq 15$ secondi, vincolo$_G$ \textit{sempre rispettato}.
    \item \textbf{Valore ottimale:} $LLMRT \leq 8$ secondi, vincolo$_G$ \textit{sempre rispettato}.
\end{itemize}
...
Il valore iniziale risente del \textit{Proof of Concept}$_G$, scritto rapidamente per dimostrare la fattibilità$_G$ e con la logica delle azioni basate sull'LLM$_G$ concentrata dentro le finestre dell'interfaccia. Il lavoro degli sprint$_G$ 9 e 10 ha spostato quella logica nei servizi del dominio$_G$ e nei moduli dedicati alla composizione dei \textit{prompt}$_G$, riducendo le diramazioni nelle singole funzioni; l'undicesimo sprint$_G$ ha aggiunto la rifinitura dei percorsi d'errore.
\begin{itemize}
    \item \textbf{Valore accettabile:} $CYC \leq 4$, vincolo$_G$ \textit{rispettato} dallo sprint$_G$ 9.
    \item \textbf{Valore ottimale:} $CYC \leq 3$, vincolo$_G$ \textit{violato}.
\end{itemize}
...
\end{table}
\subsubsection{PB (Sprint 8--11)}
I rilievi dell'analisi statica erano concentrati nel backend del \textit{Proof of Concept}$_G$, scritto rapidamente per dimostrare la fattibilità$_G$ dell'integrazione$_G$ con il servizio LLM$_G$ e mai sottoposto a una revisione sistematica, avendo l'ordinamento degli \textit{import} non conforme, righe eccedenti la lunghezza massima, eccezioni propagate senza catena di origine. Il frontend$_G$ del prototipo non ne presentava alcuno.
...
\begin{itemize}
    \item \textbf{Valore accettabile:} $CS \leq 5$ per componente, vincolo$_G$ \textit{sempre rispettato}.
    \item \textbf{Valore ottimale:} $CS \leq 1$ per componente, vincolo$_G$ \textit{rispettato} dallo sprint$_G$ 10.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Valore accettabile:} $DUP \leq 10\%$, vincolo$_G$ \textit{sempre rispettato}.
    \item \textbf{Valore ottimale:} $DUP \leq 3\%$, vincolo$_G$ \textit{sempre rispettato}.
\end{itemize}
...
Il miglioramento c'è ed è visibile nella forma del grafo delle dipendenze anziché nel suo rapporto. Nel prototipo l'unica dipendenza del backend collegava le rotte a un \textit{package}$_G$ che portava il nome di una tecnologia; nell'MVP$_G$ tutte le dipendenze convergono verso il \textit{package}$_G$ di dominio$_G$, che non dipende da nulla, ed è esattamente l'assetto deliberato nella riunione del 4 agosto 2026, che nello stesso sprint$_G$ aveva assegnato ai programmatori$_G$ la verifica$_G$ esplicita del livello di accoppiamento. Valutazione:
\begin{itemize}
    \item \textbf{Valore accettabile:} $MOD \leq 0{,}60$, vincolo$_G$ \textit{sempre rispettato}.
    \item \textbf{Valore ottimale:} $MOD \leq 0{,}45$, vincolo$_G$ \textit{violato}.
\end{itemize}
...
La validazione$_G$ degli ingressi è l'unica delle tre metriche che si muove. Nell'ottavo sprint$_G$ metà dei campi esposti dall'API$_G$ era priva di vincoli effettivi. I testi avevano un limite inferiore ma non superiore, e il collegamento fornito dall'utente veniva accettato come stringa qualunque, con un commento nel codice che rimandava esplicitamente la validazione$_G$ a un momento successivo. Il completamento del backend negli sprint$_G$ 9 e 10 e la rifinitura dell'undicesimo portano tutti i campi a essere vincolati in entrambe le direzioni, con un tipo dedicato per il collegamento. Valutazione:
\begin{itemize}
    \item \textbf{Valore accettabile:} $SK = 0$ chiavi esposte, vincolo$_G$ \textit{sempre rispettato}.
    \item \textbf{Valore ottimale:} $SK = 0$ chiavi esposte, vincolo$_G$ \textit{sempre rispettato}.
    \item \textbf{Valore accettabile:} $IV \geq 95\%$ degli input$_G$ validati, vincolo$_G$ \textit{rispettato} nello sprint$_G$ 11.
    \item \textbf{Valore ottimale:} $IV = 100\%$ degli input$_G$ validati, vincolo$_G$ \textit{rispettato} nello sprint$_G$ 11.
    \item \textbf{Valore accettabile:} $DS \geq 95\%$ dei dati sanificati, vincolo$_G$ \textit{sempre rispettato}.
    \item \textbf{Valore ottimale:} $DS = 100\%$ dei dati sanificati, vincolo$_G$ \textit{sempre rispettato}.
\end{itemize}
...
\subsubsection{RTB (Sprint 1-7)}
Analizzando l'andamento della \textit{Percentuale di Metriche Soddisfatte} (MPC-PMS) per la durata degli sprint$_G$ in Figura \ref{fig:pms}, notiamo un percorso discontinuo ma in miglioramento. A fronte di alcune criticità operative riscontrate negli Sprint$_G$ 1 e 3 in cui i nostri risultati (67\% e 75\%) sono scesi sotto la soglia di accettabilità dell'80\% per i motivi già citati in questo documento, abbiamo messo in campo un'ottima capacità di reazione. Dopo aver recuperato l'adeguatezza nello Sprint$_G$ 4 (83\%), nello sprint$_G$ 5 notiamo un completamento al 100\% delle metriche che ci porta alla piena conformità agli standard qualitativi prefissati. 
Dal calcolo sono state escluse le metriche di correttezza dei documenti, in quanto queste sono calcolate a documento approvato e non a sprint$_G$. Il loro coinvolgimento nel calcolo produrrebbe uno sfasamento che non permetterebbe una stima affidabile. Valutazione:
\begin{itemize}
    \item \textbf{Valore accettabile:} $PMS \geq 80\%$, vincolo$_G$ \textit{non rispettato} nello sprint$_G$ 1 e 3.
\end{itemize}

\subsubsection{PB (Sprint 8-12)}
Quando analizziamo l'andamento del \textit{MPC-PMS} durante la fase di PB$_G$ visibile in Figura \ref{fig:pms}, notiamo un andamento diverso da quello riscontrabile durante la fase di RTB$_G$.
Innanzitutto notiamo un calo considerevole delle performance nello sprint$_G$ 8. Questo è dovuto principalmente a due fattori:
\begin{itemize}
    \item Dalla fase di PB$_G$ il numero di metriche considerate aumenta (da 11 a 39) e questo riscala le percentuali mostrate in \textit{MPC-PMS}. Questo, quindi, vale dallo sprint$_G$ 8 in poi.
    \item In questa fase il codice proviene dalla fase di RTB$_G$ che, seppur scritto cercando il riutilizzo massimo in PB$_G$, non è perfetto (anzi).
\end{itemize}
Ciò detto, possiamo notare che durante gli sprint$_G$ successivi le percentuali di metriche soddisfatte aumentano senza interruzioni fino ad arrivare al 100\% degli ultimi due sprint$_G$. La risalita è graduale perché le quattro metriche del processo di verifica$_G$ entrano nel calcolo insieme alla codifica$_G$ dell'MVP$_G$ e si assestano solo con il suo completamento: MPC-CC rientra nella soglia dal decimo sprint$_G$ e MPC-TR raggiunge la copertura completa nell'undicesimo, quando il riallineamento della tabella dei test$_G$ di sistema$_G$ porta dentro i vincoli l'ultima metrica ancora scoperta. Valutazione:
\begin{itemize}
    \item \textbf{Valore accettabile:} $PMS \geq 80\%$, vincolo$_G$ \textit{non rispettato} negli sprint$_G$ 8 e 9.
    \item \textbf{Valore ottimale:} $PMS = 100\%$, vincolo$_G$ \textit{rispettato} negli sprint$_G$ 11 e 12.
\end{itemize}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|>{\raggedright\arraybackslash}p{3.5cm}|>{\raggedright\arraybackslash}X|>{\raggedright\arraybackslash}X|}
        ...
        Pianificazione e dimensionamento task$_G$ & Inesperienza iniziale (Sprint$_G$ 1) nell'organizzare, dimensionare e portare a compimento le task$_G$, con conseguente abbassamento del Task$_G$ Completion Rate (TCR) e scostamenti nei costi (EAC). & Migliore calibrazione del team e scomposizione delle task$_G$ in unità più piccole per facilitarne il completamento nei tempi previsti (come dimostrato nei recuperi degli sprint$_G$ successivi). \\ \hline
        
        Gestione e modellazione dei Requisiti$_G$ & Numerosi difetti di modellazione nel documento di Analisi dei Requisiti$_G$ hanno causato pesanti rallentamenti nello Sprint$_G$ 3, alzando drasticamente il Merge$_G$ Rework e l'Issue$_G$ Reopening Rate. & Organizzazione di sessioni di revisione congiunta prima della stesura definitiva per evitare corpose riscritture strutturali e minimizzare la riapertura delle issue$_G$. \\ \hline
        
        Calibrazione dell'effort economico & Tendenza ad accelerare il lavoro (Sprint$_G$ 2 e 3) accumulando anticipo sulla schedulazione ma causando frequenti sforamenti del budget$_G$ e Cost Variance negative. & Maggiore attenzione al bilanciamento tra l'Earned Value e l'Actual Cost$_G$, stabilizzando l'impiego delle ore produttive per non superare il costo pianificato (risolto nello Sprint$_G$ 5). \\ \hline
    \end{tabularx}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|>{\raggedright\arraybackslash}p{3.5cm}|>{\raggedright\arraybackslash}X|>{\raggedright\arraybackslash}X|}
        ...
        Coordinamento dei redattori in gruppi numerosi & Iniziali incomprensioni sul lavoro da svolgere a causa della mancanza di esperienza pregressa in un team così numeroso, sfociate in sovrascritture di porzioni di documenti. & Definizione di confini più rigidi nell'assegnazione$_G$ dei compiti ai singoli redattori per prevenire la sovrapposizione del lavoro e l'annullamento delle Pull Request$_G$. \\ \hline
        
        Verifica$_G$ formale e controllo logico & I verificatori si sono inizialmente affidati eccessivamente ai correttori ortografici, tralasciando discrepanze logiche (formule smentite dai dati testuali) e sviste nel "copia e incolla" delle didascalie. & Integrazione$_G$ di una checklist di verifica$_G$ semantica e strutturale (matching dei dati tra tabelle e testo, controllo incrociato delle definizioni metriche) obbligatoria per il ruolo del Verificatore$_G$. \\ \hline
    \end{tabularx}
