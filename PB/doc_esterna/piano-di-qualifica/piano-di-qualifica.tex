...
\begin{longtable}{|c|c|c|c|c|p{3.5cm}|}
    ...
    \hline
    \endhead
    2.0.0 & 2026-08-28 & - & - & E. De Piccoli & Approvazione per ingresso in baseline PB \\
    \hline
    1.8.0 & 2026-08-28 & S. Vendramin & A. Marini & - & Aggiunge le valutazioni MPC-CC, MPC-TSR, MPC-DD e MPC-TR e aggiorna MPC-PMS \\
    \hline
    1.7.0 & 2026-08-27 & S. Zecchinato & D. Facco & - & Aggiornamento metriche allo sprint 12 \\
    \hline
    1.6.0 & 2026-08-20 & A. Marini & A. Menegaldo & - & Sistema errori ortografici e di battitura \\
    \hline
    1.5.0 & 2026-08-18 & S. Zecchinato & E. De Piccoli & - & Aggiorna valutazione MPC-PMS definitiva \\
    \hline
    1.4.0 & 2026-09-17 & S. Zecchinato & E. De Piccoli & - & Aggiorna metriche di prodotto ultimi sprint \\
    \hline
    ...
    \hline
    1.1.0 & 2026-07-29 & S. Zecchinato & L. Battistella & - & Aggiorna copertura requisiti \\
    \hline
...
Le misurazioni che seguono fanno riferimento ai due periodi del progetto: il primo, fase \textbf{RTB}, che intercorre tra l'aggiudicazione del capitolato, avvenuto il 28 marzo 2026, e la consegna della RTB e il secondo, fase \textbf{PB}, che intercorre dalla valutazione di \textit{Semaforo Verde RTB} fino alla consegna PB.
...
Analizzando il grafico in Figura \ref{fig:pv-ev} tra lo Sprint 8 e lo Sprint 12, si osserva la fase di completamento del progetto, in cui sia il \textit{Planned Value} che l'\textit{Earned Value} convergono progressivamente verso il valore cumulato finale di circa 12.250,00 €. 
...
\subsubsection{PB (Sprint 8-12)}
Come per \textit{MPC-TCR}, \textit{MPC-TS} e \textit{MPC-MR}, è possibile notare in Figura \ref{fig:ir} che l'andamento dell'\textit{Issue Reopening Rate} è stabile sullo 0\%. Come per le altre metriche precedentemente citate, il motivo è riscontrabile nella maggiore maturità guadagnata nella durata del progetto. In nessuno sprint della fase PB, infatti, si è reso necessario riaprire issues per svolgere nuovamente un'attività conclusa.
Valutazione:
\begin{itemize}
    \item \textbf{Valore accettabile:} $IR \leq 15\%$, vincolo \textit{rispettato}.
    \item \textbf{Valore ottimale:} $IR = 0\%$, vincolo \textit{rispettato}.
\end{itemize}
...
\begin{table}[H]
	...
	\begin{tabular}{|c|l|p{3cm}|}
		...
		\hline
		Analisi dei Requisiti & 100 & Ottimo\\ 
		\hline
        Glossario & 71 & Ottimo \\
        \hline
        Norme di Progetto & 85 & Ottimo \\
        \hline
        Piano di Qualifica & 100 & Ottimo \\
        \hline
...
\begin{table}[H]
	...
	\begin{tabular}{|c|l|l|l|p{2.2cm}|}
		...
		\hline
		Analisi dei Requisiti & 0\% & 0,40\% & 99,60\% & Ottimo\\ 
		\hline
        Glossario & 0\% & 0,90\%& 99,10\% & Ottimo \\
        \hline
        Norme di Progetto & 0\% & 0,40\% & 99,60\% & Ottimo \\
        \hline
        Piano di Qualifica & 0\% & 0,28\% & 99,72\% & Ottimo \\
        \hline
        Piano di Progetto & 0\% & 0,22\% & 99,78\% & Ottimo \\
        \hline
...
\begin{itemize}
    \item \textbf{Valore accettabile:} $TSR \geq 95\%$, vincolo \textit{sempre rispettato}.
    \item \textbf{Valore ottimale:} $TSR = 100\%$, vincolo \textit{rispettato} dallo sprint$_G$ 9.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Valore accettabile:} $DD \leq 2$ difetti per componente, vincolo \textit{sempre rispettato}.
    \item \textbf{Valore ottimale:} $0$ difetti aperti, vincolo \textit{violato} nello sprint$_G$ 10.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Valore accettabile:} $TR = 100\%$ dei requisiti$_G$ obbligatori coperti, vincolo \textit{rispettato} nello sprint$_G$ 11.
    \item \textbf{Valore ottimale:} $TR = 100\%$ dei requisiti$_G$ coperti, vincolo \textit{rispettato} nello sprint$_G$ 11.
\end{itemize}
...
\subsubsection{PB (Sprint 8-12)}
Quando analizziamo l'andamento del \textit{MPC-PMS} durante la fase di PB visibile in Figura \ref{fig:pms}, notiamo un andamento diverso da quello riscontrabile durante la fase di RTB.
Innanzitutto notiamo un calo considerevole delle performance nello sprint 8. Questo è dovuto principalmente a due fattori:
\begin{itemize}
    \item Dalla fase di PB il numero di metriche considerate aumenta (da 11 a 39) e questo riscala le percentuali mostrate in \textit{MPC-PMS}. Questo, quindi, vale dallo sprint 8 in poi.
    \item In questa fase il codice proviene dalla fase di RTB che, seppur scritto cercando il riutilizzo massimo in PB, non è perfetto (anzi).
\end{itemize}
Ciò detto, possiamo notare che durante gli sprint successivi le percentuali di metriche soddisfatte aumentano senza interruzioni fino ad arrivare al 100\% degli ultimi due sprint. La risalita è graduale perché le quattro metriche del processo di verifica$_G$ entrano nel calcolo insieme alla codifica$_G$ dell'MVP$_G$ e si assestano solo con il suo completamento: MPC-CC rientra nella soglia dal decimo sprint e MPC-TR raggiunge la copertura completa nell'undicesimo, quando il riallineamento della tabella dei test$_G$ di sistema$_G$ porta dentro i vincoli l'ultima metrica ancora scoperta. Valutazione:
\begin{itemize}
    \item \textbf{Valore accettabile:} $PMS \geq 80\%$, vincolo \textit{non rispettato} negli sprint 8 e 9.
    \item \textbf{Valore ottimale:} $PMS = 100\%$, vincolo \textit{rispettato} negli sprint 11 e 12.
\end{itemize}
