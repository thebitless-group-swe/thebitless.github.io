...
Le misurazioni che seguono fanno riferimento al periodo che intercorre tra l'aggiudicazione del capitolato$_G$, avvenuto il 28 marzo 2026, e la consegna della RTB$_G$.
...
\begin{table}[H]
	...
	\label{tab:cd}
\end{table}
L'indice di completezza documentale è ottimo in tutti i documenti, com'è possibile notare in Figura \ref{fig:cd}. Questo perché la struttura di ogni documento in scrittura viene concordata preventivamente nel team e viene verificata ogni qual volta il documento passa alla fase di verifica$_G$. Valutazione:
\begin{itemize}
    \item \textbf{Valore accettabile:} $CD \geq 90\%$ delle sezioni previste, vincolo$_G$ \textit{rispettato}.
    \item \textbf{Valore ottimo:} $CD = 100\%$ delle sezioni previste, vincolo$_G$ \textit{rispettato}.
\end{itemize}
\begin{figure}[H]
...
Le sottosezioni che seguono riportano le metriche di qualità del prodotto (MPD) definite nel capitolo \textit{Metriche di qualità del prodotto} e riepilogate nelle Tabelle \ref{tab:metriche-funzionalita}, \ref{tab:metriche-affidabilita}, \ref{tab:metriche-usabilita}, \ref{tab:metriche-efficienza}, \ref{tab:metriche-manutenibilita} e \ref{tab:metriche-sicurezza}. La rilevazione parte dallo sprint$_G$ 8, primo sprint$_G$ della fase di \textit{Product Baseline}$_G$ e periodo in cui prende avvio lo sviluppo$_G$ dell'MVP$_G$, e si arresta allo sprint$_G$ 11.

La rilevazione si ferma all'undicesimo sprint$_G$ perché è in esso che si è concluso lo sviluppo$_G$ dell'MVP$_G$: l'ultima modifica al codice sorgente risale al 12 agosto 2026, dentro la finestra dell'undicesimo sprint$_G$. Lo sprint$_G$ successivo non ha comportato alcuna modifica al prodotto e i valori rilevati sarebbero identici a quelli dello sprint$_G$ 11, senza aggiungere alcuna informazione sull'andamento della qualità.
...
\end{itemize}
Come si osserva in Figura \ref{fig:mpd-cr}, la copertura dei requisiti$_G$ obbligatori e desiderabili cresce in modo regolare, mentre quella dei requisiti$_G$ opzionali resta costantemente nulla.
\begin{figure}[H]
...
\end{itemize}
Come si osserva in Figura \ref{fig:mpd-cllm}, la copertura delle funzioni LLM$_G$ raggiunge il valore pieno soltanto nell'ultimo sprint$_G$ rilevato.
\begin{figure}[H]
...
\begin{table}[H]
	...
	\label{tab:mpd-cmd}
\end{table}
Il rendering Markdown$_G$ era già solido nel \textit{Proof of Concept}$_G$: intestazioni, stili del testo, elenchi, citazioni, tabelle, formule matematiche e blocchi di codice venivano resi correttamente fin dal prototipo, che nasceva proprio per dimostrare la fattibilità$_G$ dell'editor.
...
\end{itemize}
Come si osserva in Figura \ref{fig:mpd-cmd}, il valore parte esattamente sulla soglia di accettabilità e la mantiene per tutta la rilevazione, raggiungendo quella ottimale nell'ultimo sprint$_G$.
\begin{figure}[H]
...
\end{itemize}
Come si osserva in Figura \ref{fig:mpd-sn}, il salvataggio e recupero delle note recupera nei due sprint$_G$ centrali il divario iniziale.
\begin{figure}[H]
...
Il completamento della persistenza interna negli sprint$_G$ 9 e 10 riduce progressivamente le occorrenze, e la campagna di verifica$_G$ dell'undicesimo sprint$_G$ non ne rileva alcuna: il contenuto viene restituito identico in tutti i casi provati. Valutazione:
\begin{itemize}
...
\end{itemize}
Come si osserva in Figura \ref{fig:mpd-dl}, le occorrenze di perdita si annullano progressivamente fino allo sprint$_G$ 11.
\begin{figure}[H]
...
\subsection{Valutazione della gestione degli errori e del recupero di sessione}
\begin{table}[H]
    \centering
	\small
    \setlength{\tabcolsep}{4pt}
	\begin{tabular}{|c|>{\centering\arraybackslash}p{2.1cm}|>{\centering\arraybackslash}p{2.2cm}|>{\centering\arraybackslash}p{2.1cm}|>{\centering\arraybackslash}p{2.2cm}|}
		\hline
		\textbf{Sprint} & \textbf{MPD-EH (\%)} & \textbf{Giudizio} & \textbf{MPD-SR (\%)} & \textbf{Giudizio} \\
		\hline
		8 & 80,0\% & Critico & 0,0\% & Critico \\
		\hline
        9 & 83,7\% & Critico & 11,7\% & Critico \\
        \hline
        10 & 87,2\% & Critico & 23,3\% & Critico \\
        \hline
        11 & 90,0\% & Accettabile & 33,3\% & Critico \\
        \hline
	\end{tabular}
	\caption{Gestione degli errori e recupero di sessione per sprint}
	\label{tab:mpd-eh-sr}
\end{table}
La gestione degli errori parte da un livello discreto ma incompleto. Il \textit{Proof of Concept}$_G$ rifiutava correttamente gli input$_G$ troppo brevi e i campi obbligatori mancanti, ma accettava senza obiezioni testi di qualunque lunghezza, non avendo alcun limite superiore, e soprattutto restituiva all'utente la struttura tecnica prodotta dalla libreria di validazione$_G$ anziché un messaggio comprensibile. Il lavoro di rifinitura dell'undicesimo sprint$_G$ ha introdotto i limiti superiori e riscritto tutte le risposte d'errore in linguaggio naturale, indicando il campo interessato e l'azione correttiva, e porta il valore alla soglia di accettabilità.

Il recupero di sessione resta il punto debole del prodotto e non raggiunge la soglia in nessuno sprint$_G$. Il motivo non è un difetto ma una conseguenza dell'architettura$_G$ scelta: l'MVP$_G$ non dispone di un database$_G$ e conserva le note nella memoria locale del browser soltanto nel momento in cui l'utente salva, senza alcun meccanismo di bozza automatica. Dopo un'interruzione imprevista viene quindi recuperato ciò che era già stato salvato, mentre le modifiche in corso al momento dell'interruzione vanno perdute, sia su una nota esistente sia su una nota appena creata. Colmare la distanza dalla soglia richiede un salvataggio automatico periodico, che non rientrava fra le priorità della \textit{Product Baseline}$_G$ e resta come intervento da pianificare. Valutazione:
\begin{itemize}
    ...
    \item \textbf{Valore accettabile:} recupero del testo nei casi previsti, vincolo$_G$ \textit{violato}.
    \item \textbf{Valore ottimale:} recupero completo e automatico, vincolo$_G$ \textit{violato}.
\end{itemize}
Come si osserva in Figura \ref{fig:mpd-eh-sr}, la gestione degli errori raggiunge la soglia di accettabilita' nell'ultimo sprint$_G$ mentre il recupero di sessione resta al di sotto di essa.
\begin{figure}[H]
    \includegraphics[width=1\textwidth]{charts/mpd-eh-sr.pdf}
    \caption{Valutazione \textit{Error Handling} e \textit{Session Recovery}}
    \label{fig:mpd-eh-sr}
\end{figure}
...
\end{itemize}
Come si osserva in Figura \ref{fig:mpd-ar}, la serie resta costantemente al di sopra di entrambe le soglie per tutta la durata della rilevazione.
\begin{figure}[H]
...
\end{itemize}
Come si osserva in Figura \ref{fig:mpd-lt}, il tempo di apprendimento cala in modo regolare e attraversa la soglia ottimale soltanto nell'ultimo sprint$_G$ rilevato.
\begin{figure}[H]
...
\begin{table}[H]
	...
	\label{tab:mpd-ne}
\end{table}
Gli errori di navigazione hanno la stessa origine del tempo di apprendimento e seguono lo stesso andamento: nell'ottavo sprint$_G$ quattro comandi su tredici non producevano l'effetto annunciato dalla propria etichetta, e ogni tentativo su di essi contava come azione errata.
...
\end{itemize}
Come si osserva in Figura \ref{fig:mpd-ne}, gli errori per scenario calano in modo regolare restando sempre entro la soglia di accettabilità, senza però annullarsi.
\begin{figure}[H]
...
\end{itemize}
Come si osserva in Figura \ref{fig:mpd-ui-fu}, la coerenza dell'interfaccia resta stabile mentre la comprensibilita' delle funzioni attraversa entrambe le soglie.
\begin{figure}[H]
...
\end{itemize}
Come si osserva in Figura \ref{fig:mpd-tempi-ui}, tracciata su scala logaritmica per poter mostrare insieme i valori e le soglie, le tre serie restano da uno a due ordini di grandezza al di sotto dei rispettivi vincoli.
\begin{figure}[H]
...
\end{itemize}
Come si osserva in Figura \ref{fig:mpd-llmrt}, il tempo di risposta del servizio LLM$_G$ cala in modo regolare fino allo sprint$_G$ 11.
\begin{figure}[H]
...
\begin{table}[H]
	...
	\begin{tabular}{|c|l|p{3cm}|}
		\hline
		\textbf{Sprint} & \textbf{Complessità ciclomatica (massimo)} & \textbf{Giudizio} \\
		\hline
		8 & 12 & Accettabile \\
		\hline
        9 & 18 & Critico \\
        ...
        \hline
	\end{tabular}
	\caption{Complessità ciclomatica massima per funzione per sprint}
	\label{tab:mpd-cyc}
\end{table}
È l'unica metrica di prodotto il cui andamento peggiora nel corso della \textit{Product Baseline}$_G$, e il dato va riportato per quello che è. Il valore indicato è il massimo rilevato fra tutte le funzioni del sistema$_G$ e non la loro media: una media nasconderebbe la singola funzione fuori scala, che è precisamente ciò che qui va reso visibile.

La crescita ha una causa individuabile nella scelta, deliberata nel decimo sprint$_G$, di disaccoppiare le finestre dell'interfaccia dagli \textit{endpoint}$_G$ del backend raccogliendo tutte le azioni basate sull'LLM$_G$ in un unico componente. La scelta ha effettivamente ridotto l'accoppiamento fra i moduli, ma ha concentrato in una sola funzione la ramificazione che prima era distribuita fra finestre separate, ciascuna con i propri parametri e i propri stati. Nell'undicesimo sprint$_G$ contribuisce anche la funzione che compone i messaggi d'errore in linguaggio naturale, scritta per soddisfare il requisito$_G$ sulla comprensibilità delle segnalazioni.

Il vincolo$_G$ risulta violato dal nono sprint$_G$ in poi. La riduzione della complessità di quel componente, che si ottiene scomponendolo nelle sue azioni senza rinunciare al punto di accesso unico, è un intervento di manutenzione$_G$ da pianificare e non un difetto da nascondere nella media. Valutazione:
\begin{itemize}
    \item \textbf{Valore accettabile:} $CYC \leq 15$, vincolo$_G$ \textit{rispettato} nello sprint$_G$ 8.
    \item \textbf{Valore ottimale:} $CYC \leq 10$, vincolo$_G$ \textit{violato}.
\end{itemize}
Come si osserva in Figura \ref{fig:mpd-cyc}, la complessita' massima cresce a partire dallo sprint$_G$ 9 e supera stabilmente la soglia di accettabilita'.
\begin{figure}[H]
...
\end{itemize}
Come si osserva in Figura \ref{fig:mpd-cs}, il numero di code smell per componente attraversa la soglia ottimale nel decimo sprint$_G$ e si annulla nell'undicesimo.
\begin{figure}[H]
...
\end{itemize}
Come si osserva in Figura \ref{fig:mpd-dup}, la duplicazione si mantiene sotto la soglia ottimale fin dal primo sprint$_G$ rilevato e si riduce di oltre l'ottanta per cento.
\begin{figure}[H]
...
\end{itemize}
Come si osserva in Figura \ref{fig:mpd-mod}, la serie è una retta collocata fra le due soglie: è la rappresentazione visiva del fatto che la metrica non registra la ristrutturazione avvenuta nel decimo sprint$_G$.
\begin{figure}[H]
...
\end{itemize}
Come si osserva in Figura \ref{fig:mpd-iv}, la validazione$_G$ degli input$_G$ recupera cinquanta punti percentuali e attraversa entrambe le soglie nell'ultimo sprint$_G$; MPD-SK e MPD-DS non sono riportate perché costanti al valore ottimale in ogni sprint$_G$.
\begin{figure}[H]
