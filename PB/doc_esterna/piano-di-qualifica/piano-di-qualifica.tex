...
\end{itemize}
Come si osserva in Figura \ref{fig:mpd-cr}, la copertura dei requisiti obbligatori e desiderabili cresce in modo regolare, mentre quella dei requisiti opzionali resta costantemente nulla.
\begin{figure}[H]
...
\end{itemize}
Come si osserva in Figura \ref{fig:mpd-cllm}, la copertura delle funzioni LLM raggiunge il valore pieno soltanto nell'ultimo sprint rilevato.
\begin{figure}[H]
...
\begin{table}[H]
	...
	\label{tab:mpd-cmd}
\end{table}
Il rendering Markdown$_G$ era già solido nel \textit{Proof of Concept}$_G$: intestazioni, stili del testo, elenchi, citazioni, tabelle, formule matematiche e blocchi di codice venivano resi correttamente fin dal prototipo, che nasceva proprio per dimostrare la fattibilità dell'editor.
...
\end{itemize}
Come si osserva in Figura \ref{fig:mpd-sn}, il salvataggio e recupero delle note recupera nei due sprint centrali il divario iniziale.
\begin{figure}[H]
...
\end{itemize}
Come si osserva in Figura \ref{fig:mpd-dl}, le occorrenze di perdita si annullano progressivamente fino allo sprint 11.
\begin{figure}[H]
...
\begin{itemize}
    ...
    \item \textbf{Valore accettabile:} recupero del testo nei casi previsti, vincolo \textit{violato}.
    \item \textbf{Valore ottimale:} recupero completo e automatico, vincolo \textit{violato}.
\end{itemize}
Come si osserva in Figura \ref{fig:mpd-eh-sr}, la gestione degli errori raggiunge la soglia di accettabilita' nell'ultimo sprint mentre il recupero di sessione resta al di sotto di essa.
\begin{figure}[H]
...
\end{itemize}
Come si osserva in Figura \ref{fig:mpd-llmrt}, il tempo di risposta del servizio LLM cala in modo regolare fino allo sprint 11.
\begin{figure}[H]
...
Il vincolo risulta violato dal nono sprint$_G$ in poi. La riduzione della complessità di quel componente, che si ottiene scomponendolo nelle sue azioni senza rinunciare al punto di accesso unico, è un intervento di manutenzione$_G$ da pianificare e non un difetto da nascondere nella media. Valutazione:
\begin{itemize}
    \item \textbf{Valore accettabile:} $CYC \leq 15$, vincolo \textit{rispettato} nello sprint$_G$ 8.
    \item \textbf{Valore ottimale:} $CYC \leq 10$, vincolo \textit{violato}.
\end{itemize}
Come si osserva in Figura \ref{fig:mpd-cyc}, la complessita' massima cresce a partire dallo sprint 9 e supera stabilmente la soglia di accettabilita'.
\begin{figure}[H]
...
\begin{table}[H]
	...
	\label{tab:mpd-cs}
\end{table}
I rilievi dell'analisi statica erano concentrati nel backend del \textit{Proof of Concept}$_G$, scritto rapidamente per dimostrare la fattibilità dell'integrazione con il servizio LLM$_G$ e mai sottoposto a una revisione sistematica: ordinamento degli \textit{import} non conforme, righe eccedenti la lunghezza massima, eccezioni propagate senza catena di origine. Il frontend del prototipo non ne presentava alcuno.
...
\end{itemize}
Come si osserva in Figura \ref{fig:mpd-iv}, la validazione degli input$_G$ recupera cinquanta punti percentuali e attraversa entrambe le soglie nell'ultimo sprint$_G$; MPD-SK e MPD-DS non sono riportate perché costanti al valore ottimale in ogni sprint$_G$.
\begin{figure}[H]
