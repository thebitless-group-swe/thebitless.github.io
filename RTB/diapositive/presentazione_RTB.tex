...
\setbeamertemplate{headline}{%
  \ifnum\thepage>1
  \vspace{4pt}%
  \hspace{14pt}\raisebox{-1pt}{\IfFileExists{logo_gruppo_senza_background.png}{\includegraphics[height=4mm]{logo_gruppo_senza_background.png}}{\scriptsize\color{sbgray}the bitless}}\hfill{\scriptsize\color{sbgray}\insertframenumber/\inserttotalframenumber}\hspace{14pt}%
  \vspace{2pt}%
...
\begin{frame}[plain]
  \vfill
  \begin{center}
    \IfFileExists{logo_gruppo_senza_background.png}{\includegraphics[height=15mm]{logo_gruppo_senza_background.png}}\\
    {\color{sbnavy}\rule{0.55\textwidth}{1.2pt}}\\[10pt]
    {\Huge\bfseries\color{sbnavy} Tecnologie \& Proof of Concept}\\[8pt]
    {\large\color{sbteal} Capitolato C6 --- Second Brain}\\[4pt]
...
\begin{frame}{Second Brain}
  \begin{block}{Visione}
    Un editor \textbf{Markdown} affiancato a un \textbf{LLM}: un secondo cervello per scrivere, migliorare e fare brainstorming.
  \end{block}
  \vspace{2pt}
  \begin{columns}[T]
    \column{0.54\textwidth}
    {\bfseries\small Requisiti obbligatori}
    \begin{itemize}
      \item[\textcolor{sbteal}{\textbullet}] {\small Editor Markdown + rendering live}
      \item[\textcolor{sbteal}{\textbullet}] {\small Accesso a un LLM sul testo}
      ...
      \item[\textcolor{sbteal}{\textbullet}] {\small Distant Writing (testo da prompt)}
    ...
    \begin{tikzpicture}[font=\scriptsize]
      ...
      % LLM
      \node[draw=sbaccent, very thick, fill=sbaccent!15, circle, minimum size=11mm, right=6mm of app, font=\scriptsize\bfseries] (llm) {LLM};
      \draw[-{Stealth[length=2mm]}, very thick, sbaccent] (app.east) -- (llm.west);
    \end{tikzpicture}\\[2pt]
    {\scriptsize\color{sbgray} editor a 2 pannelli + accesso LLM}
  \end{columns}
  \vspace{2pt}
\end{frame}
...
\begin{frame}{Architettura}
  ...
  \begin{tikzpicture}[
    ...
    \node[box, right=of web] (api) {\textbf{Backend}\\ FastAPI};
    \node[box, right=of api] (llm) {\textbf{LiteLLM}\\ Zucchetti};
    \draw[arr] (web) -- node[above, font=\scriptsize, text=sbnavy] {fetch POST} (api);
    \draw[arr] (api) -- node[above, font=\scriptsize, text=sbnavy] {httpx} (llm);
    \draw[arr, dashed] (llm.south) to[bend left=25] node[below, font=\scriptsize, text=sbnavy] {streaming SSE} (web.south);
  \end{tikzpicture}
  \end{center}
  \vspace{4pt}
  \begin{itemize}
    \item Streaming \textbf{end-to-end} con effetto macchina da scrivere
    \item Backend \textbf{stateless}: nessuna logica di dominio lato server
  \end{itemize}
\end{frame}
...
\begin{frame}{Tecnologie --- Frontend}
  \begin{itemize}
    ...
    \item \textbf{react-markdown + plugin} --- pipeline Markdown$\to$AST, anti-XSS, tabelle e LaTeX
  ...
  \begin{tikzpicture}[font=\scriptsize, node distance=7mm,
    ...
    \draw[a] (st) -- (md);
    \draw[a] (hk) -- (st);
    \draw[a, sbaccent] (hk.east) -- ++(1.6,0) node[right, text=sbgray] {API (SSE)};
  \end{tikzpicture}
  \end{center}
\end{frame}
...
\begin{frame}{Editor --- CodeMirror 6 vs alternative}
  \begin{columns}[T]
    ...
    \begin{scelto}{CodeMirror 6 \ensuremath{\checkmark}}
      {\small Leggero e modulare: highlighting, undo/redo e accessibilità nativi, API tipizzata.}
    \end{scelto}
  \end{columns}
  \vspace{6pt}
  \begin{center}
...
\begin{frame}{Tecnologie --- Backend}
  \begin{itemize}
    \item \textbf{Python 3.12 + FastAPI} --- async nativo per lo streaming LLM, OpenAPI autogenerato
    \item \textbf{Pydantic v2} --- validazione runtime, errori 422 espliciti, base per i tipi TypeScript
...
\begin{frame}{Backend --- FastAPI vs Node.js/Express}
  \begin{columns}[T]
    ...
    \begin{block}{Node.js + Express}
      {\small Ecosistema JS unico FE+BE, ma lo streaming LLM richiede gestire a mano async e backpressure; validazione e OpenAPI da aggiungere con librerie esterne.}
    \end{block}
    \column{0.5\textwidth}
    \begin{scelto}{FastAPI \ensuremath{\checkmark}}
      {\small Async nativo per stream lunghi senza bloccare; validazione Pydantic e OpenAPI integrate; dall'OpenAPI generiamo i tipi TypeScript del frontend.}
    \end{scelto}
  \end{columns}
  \vspace{6pt}
  \begin{center}
    {\color{sbnavy}\small Il cuore del progetto è lo streaming LLM: FastAPI lo gestisce nativamente.}
  \end{center}
\end{frame}

% ============ SLIDE 8 — CONFRONTO ACCESSO LLM ============
\begin{frame}{Accesso all'LLM --- astrazione vs chiamata diretta}
  \begin{columns}[T]
    \column{0.5\textwidth}
    \begin{block}{Chiamata diretta al provider}
      {\small Meno codice subito, ma le route restano accoppiate al provider concreto: cambiarlo o aggiungerne uno impone di riscriverle, e i test richiedono l'LLM reale.}
    \end{block}
    \column{0.5\textwidth}
    \begin{scelto}{LLMClient astratto + LiteLLMClient \ensuremath{\checkmark}}
      {\small Le route dipendono dall'interfaccia, non dal provider: cambiarlo = modifica di config; testabile con mock pytest senza chiamare l'LLM.}
    \end{scelto}
  \end{columns}
  \vspace{6pt}
...
\begin{frame}{Infrastruttura \& Qualità}
  \begin{columns}[T]
    ...
    \begin{itemize}
      ...
      \item Mono-repo --- frontend + backend, CI unificata
    \end{itemize}
    \column{0.5\textwidth}
    {\bfseries Qualità}
    \begin{itemize}
      \item GitHub Actions --- CI: lint, type check, test, build
      \item Test: Vitest + Playwright (FE), pytest (BE)
      \item openapi-typescript --- tipi TS dal backend, zero disallineamenti
    \end{itemize}
  \end{columns}
\end{frame}

% ============ SLIDE 10 — POC OBIETTIVO ============
\begin{frame}{Proof of Concept --- obiettivo}
  ...
  \begin{tikzpicture}[
    ...
    \node[flowstep, right=10mm of b] (c) {Output in\\ streaming};
    \draw[arr] (a) -- (b);
    \draw[arr] (b) -- (c);
  \end{tikzpicture}
  \end{center}
  \vspace{2pt}
  {\color{sbnavy}\small Le altre funzioni AI (traduzione, riscrittura, Sei Cappelli, Distant Writing) differiscono \textbf{solo per il prompt}: l'architettura resta identica.}
\end{frame}

% ============ SLIDE 11 — POC VALIDAZIONE ============
\begin{frame}{Proof of Concept --- cosa abbiamo validato}
  \begin{itemize}
    ...
    \item[\textcolor{sbaccent}{\ensuremath{\checkmark}}] Chiave API mai esposta al client
