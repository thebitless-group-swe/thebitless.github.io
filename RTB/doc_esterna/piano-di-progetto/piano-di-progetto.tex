...
\begin{table}[H]
    ...
    \begin{tabular}{|l|c|c|c|c|c|c|c|}
        ...
        D. Facco & - & 7 & - & - & - & - & 7 \\ \hline
        A. Marini & - & - & - & - & - & 9 & 9 \\ \hline
        A. Menegaldo & - & 4 & 3 & - & - & - & 7 \\ \hline
        S. Vendramin & - & - & - & - & 8 & - & 8 \\ \hline
        S. Zecchinato & 6 & - & - & - & 3 & - & 9 \\ \hline
        \rowcolor{bluBitless!8}
        \textbf{Totale per ruolo} & \textbf{6} & \textbf{11} & \textbf{3} & \textbf{18} & \textbf{11} & \textbf{9} & \textbf{58} \\
        \hline
    \end{tabular}
...
% Settori (partenza da 90°, senso orario)
\path[fill=colResp, draw=white, line width=1.5pt]
(0,0) -- (90:\r) arc[start angle=90,end angle=52.8,radius=\r] -- cycle;

\path[fill=colAmm, draw=white, line width=1.5pt]
(0,0) -- (52.8:\r) arc[start angle=52.8,end angle=-15.5,radius=\r] -- cycle;

\path[fill=colAn, draw=white, line width=1.5pt]
(0,0) -- (-15.5:\r) arc[start angle=-15.5,end angle=-34.1,radius=\r] -- cycle;

\path[fill=colPt, draw=white, line width=1.5pt]
(0,0) -- (-34.1:\r) arc[start angle=-34.1,end angle=-145.8,radius=\r] -- cycle;

\path[fill=colPr, draw=white, line width=1.5pt]
(0,0) -- (-145.8:\r) arc[start angle=-145.8,end angle=-214.1,radius=\r] -- cycle;

\path[fill=colVe, draw=white, line width=1.5pt]
(0,0) -- (-214.1:\r) arc[start angle=-214.1,end angle=-270,radius=\r] -- cycle;

% Percentuali interne
\node[font=\small\bfseries] at (71:1.75) {10,3\%};
\node[font=\small\bfseries] at (18:1.75) {19,0\%};
\node[font=\small\bfseries] at (-25:1.85) {5,2\%};
\node[font=\small\bfseries] at (-90:1.75) {31,0\%};
\node[font=\small\bfseries] at (-180:1.75) {19,0\%};
\node[font=\small\bfseries] at (-242:1.75) {15,5\%};

% Etichette esterne
% Responsabile
\fill[lineGray] (1.0,2.8) circle (1.7pt);
\draw[lineGray, line width=0.7pt] (1.0,2.8) -- (3.5,2.8) -- (5.5,2.8);
\node[anchor=south east, font=\normalsize\bfseries] at (5.5,2.87) {Responsabile};
\node[anchor=north east, font=\small\bfseries, text=lineGray] at (5.5,2.65) {6 ore};

% Amministratore
\fill[lineGray] (3.1,0.8) circle (1.7pt);
\draw[lineGray, line width=0.7pt] (3.1,0.8) -- (4.2,0.8) -- (5.5,0.8);
\node[anchor=south east, font=\normalsize\bfseries] at (5.5,0.87) {Amministratore};
\node[anchor=north east, font=\small\bfseries, text=lineGray] at (5.5,0.65) {11 ore};

% Analista
\fill[lineGray] (3.0,-0.5) circle (1.7pt);
\draw[lineGray, line width=0.7pt] (3.0,-0.5) -- (4.2,-0.5) -- (5.5,-0.5);
\node[anchor=south east, font=\normalsize\bfseries] at (5.5,-0.43) {Analista};
\node[anchor=north east, font=\small\bfseries, text=lineGray] at (5.5,-0.65) {3 ore};

% Progettista
\fill[lineGray] (0.1,-3.1) circle (1.7pt);
\draw[lineGray, line width=0.7pt] (0.1,-3.1) -- (-2.0,-3.1) -- (-5.5,-3.1);
\node[anchor=south west, font=\normalsize\bfseries] at (-5.5,-3.03) {Progettista};
\node[anchor=north west, font=\small\bfseries, text=lineGray] at (-5.5,-3.25) {18 ore};

% Programmatore
\fill[lineGray] (-2.9,-1.2) circle (1.7pt);
\draw[lineGray, line width=0.7pt] (-2.9,-1.2) -- (-4.2,-1.2) -- (-5.5,-1.2);
\node[anchor=south west, font=\normalsize\bfseries] at (-5.5,-1.13) {Programmatore};
\node[anchor=north west, font=\small\bfseries, text=lineGray] at (-5.5,-1.35) {11 ore};

% Verificatore
\fill[lineGray] (-2.1,2.4) circle (1.7pt);
\draw[lineGray, line width=0.7pt] (-2.1,2.4) -- (-3.8,2.4) -- (-5.5,2.4);
\node[anchor=south west, font=\normalsize\bfseries] at (-5.5,2.47) {Verificatore};
...
\begin{table}[H]
    ...
    \begin{tabular}{|l|c|c|c|c|}
        ...
        Responsabile di progetto & 6 & 0 & 180,00 & 0,00 \\ \hline
        Amministratore & 11 & +1 & 220,00 & +20,00 \\ \hline
        Analista & 3 & 0 & 75,00 & 0,00 \\ \hline
        Progettista & 18 & +3 & 450,00 & +75,00 \\ \hline
        Programmatore & 11 & +3 & 165,00 & +45,00 \\ \hline
        Verificatore & 9 & +1 & 135,00 & +15,00 \\ \hline

        \rowcolor{bluBitless!8}
        \textbf{Totale} & \textbf{58} & \textbf{+8} & \textbf{1.225,00} & \textbf{+75,00} \\ \hline
...
Di seguito sono riportate le ore rimanenti per la coppia risorsa-ruolo dopo lo Sprint \#4:
\begin{table}[H]
    \centering
    \renewcommand{\arraystretch}{1.4}
    \begin{tabular}{|l|c|c|c|c|c|c|c|}
        ...
        \textbf{Membro del team} & \textbf{Re} & \textbf{Am} & \textbf{An} & \textbf{Pt} & \textbf{Pr} & \textbf{Ve} & \textbf{Totale per persona} \\
        \hline
        L. Battistella & 2 & 2 & 0 & 13 & 22 & 23 & 62 \\ \hline
        E. De Piccoli & 7 & 4 & 2 & 13 & 22 & 17 & 65 \\ \hline
        D. Facco & 7 & 1 & 3 & 15 & 23 & 16 & 65 \\ \hline
        A. Marini & 3 & 1 & 0 & 22 & 23 & 14 & 63 \\ \hline
        A. Menegaldo & 7 & 4 & 0 & 17 & 22 & 15 & 64 \\ \hline
        S. Vendramin & 0 & 9 & 0 & 22 & 16 & 16 & 63 \\ \hline
        S. Zecchinato & 0 & 9 & 0 & 23 & 19 & 14 & 65 \\ \hline
        \rowcolor{bluBitless!8}
        \textbf{Totale per ruolo} & \textbf{26} & \textbf{30} & \textbf{5} & \textbf{125} & \textbf{147} & \textbf{115} & \textbf{447} \\
        \hline
    \end{tabular}
...
\begin{itemize}
    ...
    \item progettazione dell'infrastruttura del PoC: definizione della struttura a servizi, dei contratti API, del flusso di streaming e delle politiche di gestione delle credenziali;
    \item introduzione formale delle attività del Programmatore, avviate in sinergia collaborativa con i Progettisti;
    \item avvio del PoC: è stata completata la fase di setup infrastrutturale, con la creazione del repository condiviso, configurazione dei container con Docker Compose, protezione delle credenziali, struttura base del frontend e del backend, e regole per il dialogo tra i due servizi;
    \item avvio del percorso di formazione e allineamento interno sulle tecnologie selezionate;
    \item avanzamento e stesura dei documenti di supporto, tra cui il \textit{Piano di Progetto}, il \textit{Piano di Qualifica} e il Glossario.
\end{itemize}

\subsubsection{Retrospettiva}
Di seguito sono riportate le analisi a posteriori del quarto sprint\textsubscript{G}, emerse durante la riunione di retrospettiva del team:

\begin{itemize}
    \item Il principale scostamento rispetto al preventivo ha riguardato la sottostima del carico di lavoro reale. L'esperienza degli sprint precedenti, in cui le attività erano prevalentemente documentali, aveva portato a ipotizzare un fabbisogno orario simile anche per questo sprint. Tuttavia, la compresenza di documentazione da consolidare e l'avvio effettivo dello sviluppo del PoC ha reso evidente, già dopo i primi giorni, che le ore preventivate non fossero sufficienti.

    \item In particolare, il ruolo di \textbf{Progettista} ha richiesto un incremento di 3 ore complessive rispetto al piano. La necessità di comprendere a fondo le tecnologie selezionate, definire con precisione le classi da implementare e tradurre i requisiti in specifiche architetturali chiare per il PoC ha assorbito più tempo del previsto. Questa attività di progettazione di dettaglio, ha prodotto una base solida che faciliterà gli sprint successivi.

    \item Anche il ruolo di \textbf{Programmatore} ha evidenziato una discrepanza significativa: a fronte delle 8 ore preventivate per un unico programmatore, il consuntivo ha registrato 11 ore complessive. La concentrazione dell'intero carico di sviluppo su una sola persona, unita alla complessità della Fase 1 del PoC, ha reso necessario un intervento straordinario del Responsabile, che ha dedicato 3 ore extra all'attività di programmazione per affiancare il Programmatore designato e garantire il completamento del setup infrastrutturale entro la finestra dello sprint. Questo intervento, sebbene non pianificato, si è rivelato decisivo per rispettare gli obiettivi prefissati.
    \item Anche il \textbf{Verificatore} ha visto un lieve incremento (da 8 a 9 ore), dovuto all'aumento degli artefatti da controllare: non solo documenti aggiornati, ma anche i primi file di configurazione e il codice del PoC. L'attività di verifica preventiva su questi ultimi è stata comunque utile per evitare l'accumulo di debito tecnico nei cicli futuri.

    \item Il team ha riscontrato un feedback ampiamente positivo per quanto riguarda l'organizzazione e la conduzione delle riunioni di coordinamento, che si sono confermate momenti efficaci di allineamento e condivisione.

\end{itemize}

\subsubsection{Aggiornamento della pianificazione e del preventivo}
Alla luce di quanto emerso nella retrospettiva, il team ha definito un piano d'azione per migliorare l'organizzazione e la produttività dello sprint successivo:

\begin{itemize}
    \item \textbf{Ricalibrazione delle stime}: il preventivo del prossimo sprint terrà conto della maggiore incidenza oraria dei ruoli di Progettista e Programmatore, prevedendo un buffer aggiuntivo per assorbire eventuali rallentamenti legati alla curva di apprendimento delle tecnologie.
    \item \textbf{Previsione di un secondo Programmatore}: lo sprint futuro assegnerà ore di programmazione ad almeno due membri del team, in modo da distribuire il lavoro e favorire il trasferimento incrociato di competenze.
    \item \textbf{Affiancamento strutturato}: verranno mantenuti e potenziati i micro-gruppi composti da Progettisti e Programmatori, che in questo sprint hanno dimostrato di accelerare l'allineamento tecnico e la risoluzione dei problemi.
    \item \textbf{Tracciamento più granulare}: a integrazione dell'ITS, verranno adottate checklist di avanzamento per ogni issue complessa, con l'obiettivo di rendere più visibili i progressi e individuare prima eventuali blocchi.
    \item \textbf{Monitoraggio rafforzato}: il Responsabile dedicherà un momento fisso a metà sprint alla verifica dell'andamento delle ore, per intervenire tempestivamente con eventuali riallocazioni prima che gli scostamenti diventino critici.
\end{itemize}

\paragraph{Pianificazione futura}
A seguito di quanto emerso nella retrospettiva, il team ha riorientato la pianificazione del prossimo sprint su alcuni correttivi mirati, che toccano sia l'allocazione delle risorse sia l'organizzazione del lavoro.

\begin{itemize}
    \item Per lo sprint successivo il team prevede quindi \textbf{due Programmatori}, che opereranno in parallelo su due fronti distinti ma coordinati: il frontend (interfaccia utente, gestione dello stato, editor e anteprima) e il backend (validazione input, endpoint, streaming e gestione errori). Distribuire il lavoro riduce la dipendenza da singole persone e rende il gruppo più adattabile agli imprevisti.

    \item La fase di progettazione di dettaglio, che nello sprint precedente ha assorbito più tempo del previsto, continuerà anche nello sprint successivo. Tuttavia, ora il grosso delle decisioni architetturali è stato preso, e l'attività dei Progettisti si concentrerà sull'affiancamento dei Programmatori nelle scelte implementative più delicate, come l'astrazione del provider LLM e la gestione dello streaming SSE.

    \item La retrospettiva ha fatto emergere con chiarezza che lo studio individuale, quando condotto in modo isolato, produce ritardi e disallineamenti. Per il prossimo sprint il team ha quindi deciso di cambiare approccio: i Programmatori si coordineranno in riunioni operative dedicate per organizzare insieme il lavoro, suddividere i task e definire le interfacce tra frontend e backend prima di iniziare a scrivere codice. Allo stesso modo, i Progettisti continueranno ad affiancare i Programmatori nelle scelte implementative più critiche, mentre il Responsabile e l'Amministratore lavoreranno a stretto contatto per mantenere allineati la pianificazione e il tracciamento delle attività.
    \item A differenza degli sprint precedenti, in cui la documentazione era l'attività prevalente, il prossimo periodo vedrà una convivenza stabile tra stesura dei documenti e sviluppo del PoC. La pianificazione terrà conto di questa compressenza, assegnando ore adeguate a entrambe le aree senza sacrificare la qualità di nessuna delle due.
\end{itemize}

In sintesi, la pianificazione del prossimo sprint include le lezioni apprese in un assetto organizzativo più robusto: due programmatori, progettisti in ruolo di supporto tecnico, apprendimento integrato e monitoraggio più frequente. L'obiettivo non è fare di più, ma farlo con maggiore consapevolezza, migliorando la prevedibilità del lavoro.
\paragraph{Preventivo a finire}
